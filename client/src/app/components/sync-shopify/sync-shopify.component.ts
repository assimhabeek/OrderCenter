...
@Component({
  ...
  templateUrl: './sync-shopify.component.html',
  styleUrls: ['./sync-shopify.component.scss']
})
export class SyncShopifyComponent {
  ...
  refresh(): void {
    this.syncOrdersService.getShopifyOrders().subscribe(res => {
      if (res.status) {
